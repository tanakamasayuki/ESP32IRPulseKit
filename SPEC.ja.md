...
ACフレームは長いため、自走ハードウェアキャリア（`setPhaseAlignedCarrier(false)`）で送信します。位相整合キャリア（§6.5）は各マークをキャリア1周期ずつのシンボルで描画するため、複数フレームのACバーストではRMTのシンボル上限を超えます。ハードウェアキャリアは必要シンボル数がはるかに少なく、この長さのフレームに収まる唯一のキャリアモードです。

トレードオフとして、ハードウェアキャリアはマークごとにキャリア位相をリセットしないため、長尺フレームの1フレームあたり到達率は、短尺リモコンが使う位相整合キャリアよりわずかに低くなります。受信できたフレームは常にバイト正確（チェックサム検証済み）ですが、まれに1フレーム丸ごと受信側で検出されないことがあります。長尺フレーム向けの位相整合キャリア対応は未提供です。
