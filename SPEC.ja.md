...
### 0.6 コメント記法ルール（コード/サンプル）
- ライブラリ実装内のコメントは英語のみとする。
- サンプルコード内のコメントは英語/日本語の併記を徹底する。
  - 複数行の説明コメントは次の2行形式で記述する。
    - `// en: English sentence...`
    - `// ja: 日本語訳...`
  - 1行の短い補足コメントは `/` 区切りで併記する。
    - 例: `// en: short note / ja: 短い説明`

### 0.7 ログ出力方針
- ESP-IDF の `ESP_LOGx` 系を利用する（`ESP_LOGE/W/I/D/V`）。Arduino IDE 側でログレベル設定済みであることを前提とする。
- ログレベルごとの出力内容例と使用指針:
  - `E`（Error）：致命的・回復不能なエラー。begin 失敗、RMT チャネル確保失敗、引数不正で処理を中断するケース等。
  - `W`（Warning）：処理は継続するが望ましくない状態。truncate 発生、repeat 未対応のフレーム受信、容量不足による候補ドロップ等。
  - `I`（Info）：通常動作の状態遷移や設定。begin/end、プロトコル自動登録、idle threshold 決定など主要イベント。
  - `D`（Debug）：開発時の診断に有用な詳細情報。decode 成功/失敗の理由、score、タイミング許容差、送信要求の内容など。
  - `V`（Verbose）：生データや大量出力を伴う詳細トレース。RMT 受信シンボル列、RAW tick 配列、エンコード生成結果など通常時は抑制すべき内容。デバッグ限定で使用する。
- デフォルト運用では `D` までを実用ログとし、`V` は問題解析時のみ有効化する方針とする。
