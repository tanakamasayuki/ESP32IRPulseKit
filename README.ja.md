...
本ライブラリは短い民生リモコンのフレーム（NEC・Sony・AEHA など）を対象とし、意味のあるビットへのデコード、類似プロトコルの判別、再送を行います。エアコン／ヒートポンプ系プロトコル（Daikin・Mitsubishi-AC・Panasonic-AC など、1ボタンで多バイトの状態フレーム全体を送るもの。別のモデリング層が必要）は**対象外**で、あらゆるマイナープロトコルを網羅することも目指しません。対応プロトコル数の網羅性は目標ではなく、学習・再送だけでよければRAWキャプチャ/再送が任意の波形を扱えます。
