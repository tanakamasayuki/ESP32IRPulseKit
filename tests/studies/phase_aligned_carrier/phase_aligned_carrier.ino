// High-resolution RMT receiver for the phase-aligned-carrier A/B probe.
//
// Same 1 us (1 MHz) capture path as studies/carrier_jitter: dump every captured
// edge duration in microseconds (`RX_JITTER seq=.. len=.. us=..`). Here the peer
// (peer_tx/) sends a NEC-shaped frame through the library IRSender in either the
// hardware (free-running) or phase-aligned carrier mode; this side measures how
// stable the demodulated bit-marks are so the study can compare the two.
#include <Arduino.h>
