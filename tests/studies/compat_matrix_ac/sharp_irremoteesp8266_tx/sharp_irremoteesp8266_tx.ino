// Compat-AC field-map calibration (Sharp) -- RX side (ESP32IRPulseKit).
//
// The peer TX (peer_tx/) is IRremoteESP8266, which encodes a KNOWN Sharp A/C state
// (standard 13-byte SHARP_AC, A907 model) to its canonical bytes and transmits it.
// This primary captures that burst (LSB-first, single frame, fixed AA 5A CF 10
// header) with our RAW path and decodes it with esp32irpk::ac::Sharp, printing both
// the recovered 13 bytes and our interpreted fields. The harness compares our bytes
// vs the peer's canonical bytes and our fields vs the known state.
...
{
  esp32irpk::ac::Sharp::Frame f;
  if (!esp32irpk::ac::Sharp::Frame::fromRaw(raw, f))
  {
...
void setup()
{
  ...
  rx.setMaxRxSymbols(512);       // single 13-byte frame (~110 symbols)
  rx.setIdleThresholdUs(100000); // > 29ms inter-frame gap upper bound
  if (!rx.begin())
  {
