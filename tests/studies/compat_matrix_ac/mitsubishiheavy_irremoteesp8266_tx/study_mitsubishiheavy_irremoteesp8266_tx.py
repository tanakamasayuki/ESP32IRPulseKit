...
# AC decoder calibration (Mitsubishi Heavy): an INDEPENDENT transmitter
# (IRremoteESP8266's IRMitsubishiHeavy152Ac) encodes a known 19-byte
# MITSUBISHI_HEAVY_152 state and transmits it; our RX (ESP32IRPulseKit,
# esp32irpk::ac::MitsubishiHeavy) captures and decodes it. Proves our fromRaw()
# reads a third-party Mitsubishi Heavy burst and recovers the same bytes + fields
# (including the inverted byte pairs and the shorter-1-space encoding).
#
...
#
# Both stacks use the same field codes (mode: auto0/cool1/dry2/fan3/heat4;
# fan: auto0/low1/med2/high3/max4). Temperature range is 17-31 C.
...
OUR_FAN = {"AUTO": 0, "LOW": 1, "MED": 2, "HIGH": 3, "MAX": 4}

# 19 bytes = 38 hex chars. The trailing newline anchor is required so pexpect waits
# for the whole line instead of matching a partial hex prefix from the first chunk.
TX_OK_AC = re.compile(rb"TX_OK_AC vendor=MITSUBISHI_HEAVY bytes=(?P<bytes>[0-9A-Fa-f]{38})\r?\n")
AC_DECODE = re.compile(
    rb"AC_DECODE vendor=MITSUBISHI_HEAVY checksum=(?P<checksum>ok|bad) "
    rb"power=(?P<power>\d+) mode=(?P<mode>\d+) temp=(?P<temp>\d+) "
    rb"fan=(?P<fan>\d+) bytes=(?P<bytes>[0-9A-Fa-f]{38})\r?\n"
)
...
def test_mitsubishiheavy_irremoteesp8266_tx(dut, peers, case, record_property):
