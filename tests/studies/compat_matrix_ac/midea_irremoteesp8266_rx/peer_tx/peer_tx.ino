// Compat-AC encoder verification (Midea) -- TX side (ESP32IRPulseKit).
//
// Builds a known Midea A/C state (48-bit / 6-byte) with esp32irpk::ac::Midea from
// the harness command, renders it to RAW with toRaw() (data + inverted copy), and
// transmits it. Echoes the exact 6 bytes it sent (recovered via our own fromRaw)
// so the harness can compare against what the IRremoteESP8266 primary decodes.
...
using Frame = esp32irpk::ac::Midea::Frame;
using Mode = esp32irpk::ac::Midea::Mode;
using Fan = esp32irpk::ac::Midea::Fan;
