// Compat-AC encoder verification (Haier) -- TX side (ESP32IRPulseKit).
//
// Builds a known Haier A/C state (9-byte HAIER_AC) with esp32irpk::ac::Haier from
// the harness command, renders it to RAW with toRaw(), and transmits it. Echoes the
// exact 9 bytes it sent (recovered via our own fromRaw) so the harness can compare
// against what the IRremoteESP8266 primary decodes. Peer name "tx" reuses
// TEST_SERIAL_PORT_PEER_TX_TX_ESP32S3.
//
// Carrier is phase-aligned (the library default). Power is the On/Off command, so
// setPower is called last (see the vendor header).
//
...
using Frame = esp32irpk::ac::Haier::Frame;
using Mode = esp32irpk::ac::Haier::Mode;
using Fan = esp32irpk::ac::Haier::Fan;
...
void handleSendAc(const String &line)
{
  ...
  f.setFan(fan);
  f.setPower(fieldValue(line, "power").toInt() != 0); // command last
