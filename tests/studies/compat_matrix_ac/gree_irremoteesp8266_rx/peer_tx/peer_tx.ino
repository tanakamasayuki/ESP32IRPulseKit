// Compat-AC encoder verification (Gree) -- TX side (ESP32IRPulseKit).
//
// Builds a known Gree A/C state with esp32irpk::ac::Gree from the harness
// command, renders it to RAW with toRaw(), and transmits it. The peer echoes the
// exact 8 bytes it sent (recovered via our own fromRaw, which the
// gree_irremoteesp8266_tx variant verified byte-for-byte) so the harness can
...
using Frame = esp32irpk::ac::Gree::Frame;
using Mode = esp32irpk::ac::Gree::Mode;
using Fan = esp32irpk::ac::Gree::Fan;
