// Compat-AC encoder verification (Carrier) -- TX side (ESP32IRPulseKit).
//
// Builds a known Carrier A/C state (8-byte / 64-bit CARRIER_AC64) with
// esp32irpk::ac::Carrier from the harness command, renders it to RAW with toRaw(),
// and transmits it. Echoes the exact 8 bytes it sent (recovered via our own
// fromRaw) so the harness can compare against what the IRremoteESP8266 primary
...
using Frame = esp32irpk::ac::Carrier::Frame;
using Mode = esp32irpk::ac::Carrier::Mode;
using Fan = esp32irpk::ac::Carrier::Fan;
