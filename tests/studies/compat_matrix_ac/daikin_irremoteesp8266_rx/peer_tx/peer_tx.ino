// Compat-AC encoder verification (Daikin) -- TX side (ESP32IRPulseKit).
//
// Builds a known Daikin A/C state (classic ARC433) with esp32irpk::ac::Daikin from
// the harness command, renders it to RAW with toRaw() (a 5-bit preamble + three
// sections), and transmits it. The peer echoes the exact 35 bytes it sent
// (recovered via our own fromRaw, which the daikin_irremoteesp8266_tx variant
// verified byte-for-byte) so the harness can compare them against what the
// IRremoteESP8266 primary decodes. The peer name is "tx" so it reuses
// TEST_SERIAL_PORT_PEER_TX_TX_ESP32S3.
//
// Carrier mode is selectable. Daikin's zero-space (428us) EQUALS its bit mark
// (428us) -- no margin for wobble -- so the build-time default is phase-aligned
// (PULSEKIT_CARRIER 1).
//
// Commands:
//   SEND_AC mode=<AUTO|COOL|DRY|FAN|HEAT> fan=<AUTO|QUIET|MIN|LOW|MED|HIGH|MAX> temp=<C> swingv=<0|1> swingh=<0|1> power=<0|1>
//   CARRIER <pa|hw>
...
// 0 = free-running hardware carrier, 1 = phase-aligned carrier (Daikin default).
...
using Frame = esp32irpk::ac::Daikin::Frame;
using Mode = esp32irpk::ac::Daikin::Mode;
using Fan = esp32irpk::ac::Daikin::Fan;
...
bool mapFan(const String &name, Fan &fan)
{
  ...
  if (name == "MIN")   { fan = Fan::MIN_SPEED;  return true; }
...
void handleSendAc(const String &line)
{
  ...
  // Recover the exact transmitted bytes (signatures/checksums stamped by toRaw) by
