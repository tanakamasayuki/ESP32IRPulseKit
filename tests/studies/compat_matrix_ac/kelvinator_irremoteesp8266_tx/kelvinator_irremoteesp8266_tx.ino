// Compat-AC field-map calibration (Kelvinator) -- RX side (ESP32IRPulseKit).
//
// The peer TX (peer_tx/) is IRremoteESP8266's IRKelvinatorAC, which encodes a KNOWN
// Kelvinator state (16-byte, two blocks) to its canonical bytes and transmits it.
// This primary captures the burst (two 8-byte blocks, each with a B010 command
// footer + gaps) with our RAW path and decodes it with esp32irpk::ac::Kelvinator,
// printing the recovered 16 bytes and our interpreted fields. The harness compares
// our bytes vs the peer's canonical bytes and our fields vs the known state.
...
{
  esp32irpk::ac::Kelvinator::Frame f;
  if (!esp32irpk::ac::Kelvinator::Frame::fromRaw(raw, f))
  {
  ...
  Serial.println();
  Serial.flush(); // 16-byte line is long; drain to avoid ring-buffer truncation
}
} // namespace
...
void setup()
{
  ...
  rx.setMaxRxSymbols(1024);      // two 8-byte blocks + footers/gaps
  rx.setIdleThresholdUs(100000); // 100ms > the ~40ms inter-block gap
  if (!rx.begin())
  {
