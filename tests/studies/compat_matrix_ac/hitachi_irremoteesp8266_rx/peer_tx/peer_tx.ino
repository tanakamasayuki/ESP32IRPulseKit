// Compat-AC encoder verification (Hitachi) -- TX side (ESP32IRPulseKit).
//
// Builds a known Hitachi A/C state (28-byte HITACHI_AC) with esp32irpk::ac::Hitachi
// from the harness command, renders it to RAW with toRaw(), and transmits it.
// Echoes the exact 28 bytes it sent (recovered via our own fromRaw) so the harness
// can compare against what the IRremoteESP8266 primary decodes. Peer name "tx"
...
using Frame = esp32irpk::ac::Hitachi::Frame;
using Mode = esp32irpk::ac::Hitachi::Mode;
using Fan = esp32irpk::ac::Hitachi::Fan;
