// Compat-AC encoder verification (Sharp) -- TX side (ESP32IRPulseKit).
//
// Builds a known Sharp A/C state (standard 13-byte SHARP_AC, A907 model) with
// esp32irpk::ac::Sharp from the harness command, renders it to RAW with toRaw()
// (LSB-first, single frame), and transmits it. Echoes the exact 13 bytes it sent
// (recovered via our own fromRaw) so the harness can compare against what the
// IRremoteESP8266 primary decodes. Peer name "tx" reuses
// TEST_SERIAL_PORT_PEER_TX_TX_ESP32S3.
//
// Carrier defaults to phase-aligned for consistency with the other AC studies
// (Sharp's zero-space 580us > bit mark 470us, so it is not in the tight-margin
// class; HW carrier would also work).
//
// Commands:
//   SEND_AC mode=<AUTO|HEAT|COOL|DRY> fan=<AUTO|MIN|MED|HIGH|MAX> temp=<C> power=<0|1>
//   CARRIER <pa|hw>
...
using Frame = esp32irpk::ac::Sharp::Frame;
using Mode = esp32irpk::ac::Sharp::Mode;
using Fan = esp32irpk::ac::Sharp::Fan;
