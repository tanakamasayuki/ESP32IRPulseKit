// Compat-AC encoder verification (Kelvinator) -- TX side (ESP32IRPulseKit).
//
// Builds a known Kelvinator A/C state (16-byte, two blocks) with
// esp32irpk::ac::Kelvinator from the harness command, renders it to RAW with
// toRaw(), and transmits it. Echoes the exact 16 bytes it sent (recovered via our
...
//
// Carrier is phase-aligned (the library default; the two-block Kelvinator burst is
// long, and PA keeps the marks stable frame-to-frame).
//
...
using Frame = esp32irpk::ac::Kelvinator::Frame;
using Mode = esp32irpk::ac::Kelvinator::Mode;
using Fan = esp32irpk::ac::Kelvinator::Fan;
