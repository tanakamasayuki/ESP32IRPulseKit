...
# AC encoder verification (Toshiba): our TX (ESP32IRPulseKit, esp32irpk::ac::Toshiba)
# encodes a known standard 9-byte TOSHIBA_AC state (MSB-first) and transmits it; an
# INDEPENDENT receiver (IRremoteESP8266's IRrecv + IRToshibaAC) decodes it. Proves
...
#
# Peer TX defaults to the phase-aligned carrier (Toshiba zero-space 490us < bit
# mark 580us). Note: IRremoteESP8266's getFan() returns a SPEED index (0=auto,
# 1..5), not the wire field, so OUR_FAN here is the speed index.
...
def test_toshiba_irremoteesp8266_rx(dut, peers, case, record_property):
