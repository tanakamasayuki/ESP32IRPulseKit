// Compat-AC encoder verification (Toshiba) -- TX side (ESP32IRPulseKit).
//
// Builds a known Toshiba A/C state (standard 9-byte TOSHIBA_AC) with
// esp32irpk::ac::Toshiba from the harness command, renders it to RAW with toRaw()
// (MSB-first, single frame), and transmits it. Echoes the exact 9 bytes it sent
...
//
// Carrier defaults to phase-aligned (Toshiba zero-space 490us < bit mark 580us).
//
// Commands:
//   SEND_AC mode=<AUTO|COOL|DRY|FAN|HEAT> fan=<AUTO|MIN|LOW|MED|HIGH|MAX> temp=<C> power=<0|1>
//   CARRIER <pa|hw>
...
using Frame = esp32irpk::ac::Toshiba::Frame;
using Mode = esp32irpk::ac::Toshiba::Mode;
using Fan = esp32irpk::ac::Toshiba::Fan;
