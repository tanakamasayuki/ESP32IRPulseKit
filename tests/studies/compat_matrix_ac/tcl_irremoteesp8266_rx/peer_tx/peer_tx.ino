// Compat-AC encoder verification (TCL) -- TX side (ESP32IRPulseKit).
//
// Builds a known TCL A/C state (14-byte TCL112AC) with esp32irpk::ac::Tcl from the
// harness command, renders it to RAW with toRaw(), and transmits it. Echoes the
...
//
// Carrier is phase-aligned (the library default). setFan is called after setMode so
// the requested fan wins over setMode(FAN)'s fan=high side effect.
//
...
using Frame = esp32irpk::ac::Tcl::Frame;
using Mode = esp32irpk::ac::Tcl::Mode;
using Fan = esp32irpk::ac::Tcl::Fan;
...
void handleSendAc(const String &line)
{
  ...
  f.setMode(mode);          // may force fan=high for FAN mode
  f.setFan(fan);            // overrides so the requested fan wins
  f.setTemperatureC(fieldValue(line, "temp").toFloat());
