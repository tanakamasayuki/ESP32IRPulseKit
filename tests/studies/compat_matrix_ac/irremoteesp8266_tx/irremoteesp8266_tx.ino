// Compat-AC field-map calibration -- RX side (ESP32IRPulseKit).
//
// The peer TX (peer_tx/) is IRremoteESP8266, which encodes a KNOWN Panasonic A/C
// state to its canonical 27 bytes and transmits it. This primary captures that
// burst with our RAW path and decodes it with esp32irpk::ac::Panasonic, printing
// both the recovered 27 bytes and our interpreted fields. The harness compares:
//   1. our bytes vs the peer's canonical bytes -> validates our RAW capture and
//      Kaseikyo two-section framing, independent of how we interpret the bytes;
//   2. our decoded fields vs the known sent state -> calibrates the PROVISIONAL
//      field map (byte/bit positions and mode/fan codes) in src/ac/Panasonic.h.
//
// AC receive mode = RAW-only (setDecodeCandidates(0)) + enlarged capture
// (setMaxRxSymbols) + a long idle so the two sections land in one capture.
...
// Decode the captured RAW with our Panasonic backend and emit one AC_DECODE
// line. mode/fan are our normalized esp32irpk::ac::Panasonic::Mode / Fan
// underlying values (the codes under calibration); bytes is the full recovered
// state so the harness can diff it against the peer's canonical bytes.
