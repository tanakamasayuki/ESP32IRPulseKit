...
# AC field-map calibration (Gree): an external transmitter (IRremoteESP8266's
# IRGreeAC) sends a KNOWN Gree A/C state; our RX (ESP32IRPulseKit) captures it
# RAW and decodes it with esp32irpk::ac::Gree.
#
# Two things are checked per case:
#   * HARD (pass/fail): our recovered 8 bytes equal the peer's canonical 8 bytes,
#     with a valid checksum. This proves our RAW capture and the two-block
#     (no-header second block) framing are correct, independent of field
#     interpretation.
...
#     means the field map in src/ac/Gree.h needs adjusting -- it is reported, not
...
# Expected esp32irpk::ac::Gree enum underlying values for each symbolic state we
# ask the peer to send. Used only for the SOFT field comparison printout.
...
def test_gree_irremoteesp8266_tx(dut, peers, case, record_property):
