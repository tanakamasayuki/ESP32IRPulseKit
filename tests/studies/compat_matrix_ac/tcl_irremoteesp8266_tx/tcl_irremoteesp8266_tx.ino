// Compat-AC field-map calibration (TCL) -- RX side (ESP32IRPulseKit).
//
// The peer TX (peer_tx/) is IRremoteESP8266's IRTcl112Ac, which encodes a KNOWN TCL
// state (14-byte TCL112AC) to its canonical bytes and transmits it. This primary
// captures the burst with our RAW path and decodes it with esp32irpk::ac::Tcl,
// printing the recovered 14 bytes and our interpreted fields. The harness compares
...
{
  esp32irpk::ac::Tcl::Frame f;
  if (!esp32irpk::ac::Tcl::Frame::fromRaw(raw, f))
  {
...
void setup()
{
  ...
  rx.setMaxRxSymbols(1024);      // single 112-bit frame
  rx.setIdleThresholdUs(100000); // 100ms message gap (> the 1.65ms header space)
  if (!rx.begin())
  {
