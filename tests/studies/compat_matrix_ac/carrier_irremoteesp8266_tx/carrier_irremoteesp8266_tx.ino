// Compat-AC field-map calibration (Carrier) -- RX side (ESP32IRPulseKit).
//
// The peer TX (peer_tx/) is IRremoteESP8266's IRCarrierAc64, which encodes a KNOWN
// Carrier state (8-byte / 64-bit) to its canonical bytes and transmits it. This
// primary captures the single pulse-distance burst with our RAW path and decodes
// it with esp32irpk::ac::Carrier, printing the recovered 8 bytes and our
...
{
  esp32irpk::ac::Carrier::Frame f;
  if (!esp32irpk::ac::Carrier::Frame::fromRaw(raw, f))
  {
...
void setup()
{
  ...
  rx.setMaxRxSymbols(1024);      // single 64-bit frame
