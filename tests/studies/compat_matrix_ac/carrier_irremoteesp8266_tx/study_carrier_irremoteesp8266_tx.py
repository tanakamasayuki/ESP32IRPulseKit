...
# AC decoder calibration (Carrier): an INDEPENDENT transmitter (IRremoteESP8266's
# IRCarrierAc64) encodes a known standard 8-byte / 64-bit CARRIER_AC64 state and
# transmits it; our RX (ESP32IRPulseKit, esp32irpk::ac::Carrier) captures and
# decodes it. Proves our fromRaw() reads a third-party Carrier burst and recovers
# the same bytes + fields.
#
...
#
# Both stacks use the same field codes (mode: heat1/cool2/fan3 -- no auto/dry;
# fan: auto0/low1/med2/high3). CARRIER_AC64 carries temperature in every mode.
...
OUR_MODE = {"HEAT": 1, "COOL": 2, "FAN": 3}
OUR_FAN = {"AUTO": 0, "LOW": 1, "MED": 2, "HIGH": 3}

# 8 bytes = 16 hex chars. The trailing newline anchor is required so pexpect waits
# for the whole line instead of matching a partial hex prefix from the first chunk.
TX_OK_AC = re.compile(rb"TX_OK_AC vendor=CARRIER bytes=(?P<bytes>[0-9A-Fa-f]{16})\r?\n")
AC_DECODE = re.compile(
    rb"AC_DECODE vendor=CARRIER checksum=(?P<checksum>ok|bad) "
    rb"power=(?P<power>\d+) mode=(?P<mode>\d+) temp=(?P<temp>\d+) "
    rb"fan=(?P<fan>\d+) bytes=(?P<bytes>[0-9A-Fa-f]{16})\r?\n"
)
...
def test_carrier_irremoteesp8266_tx(dut, peers, case, record_property):
