// Compat-AC encoder verification (Mitsubishi) -- TX side (ESP32IRPulseKit).
//
// Builds a known Mitsubishi A/C state with esp32irpk::ac::Mitsubishi from the
// harness command, renders it to RAW with toRaw() (the frame twice, as a real
// remote does), and transmits it. The peer echoes the exact 18 bytes it sent
// (recovered via our own fromRaw, which the mitsubishi_irremoteesp8266_tx variant
// verified byte-for-byte) so the harness can compare them against what the
// IRremoteESP8266 primary decodes.
//
// Carrier mode is selectable (study_carrier_ab.py). Like Gree, Mitsubishi's
// zero-space (420us) is shorter than its bit mark (450us), so the phase-aligned
// carrier is the reliable choice and the build-time default here is
// phase-aligned (PULSEKIT_CARRIER 1).
//
// Commands:
//   SEND_AC mode=<AUTO|COOL|HEAT|DRY|FAN> fan=<AUTO|QUIET|LOW|MED|HIGH|MAX> temp=<C> power=<0|1>
//   CARRIER <pa|hw>
...
// 0 = free-running hardware carrier, 1 = phase-aligned carrier (Mitsubishi default).
...
using Frame = esp32irpk::ac::Mitsubishi::Frame;
using Mode = esp32irpk::ac::Mitsubishi::Mode;
using Fan = esp32irpk::ac::Mitsubishi::Fan;
