// Compat-AC field-map calibration (Haier) -- RX side (ESP32IRPulseKit).
//
// The peer TX (peer_tx/) is IRremoteESP8266's IRHaierAC, which encodes a KNOWN
// Haier state (9-byte HAIER_AC) to its canonical bytes and transmits it. This
// primary captures the double-header burst with our RAW path and decodes it with
// esp32irpk::ac::Haier, printing the recovered 9 bytes and our interpreted fields.
// The harness compares our bytes vs the peer's canonical bytes and our fields vs
// the known state.
...
{
  esp32irpk::ac::Haier::Frame f;
  if (!esp32irpk::ac::Haier::Frame::fromRaw(raw, f))
  {
...
void setup()
{
  ...
  rx.setMaxRxSymbols(1024);      // single 72-bit frame + double header
  rx.setIdleThresholdUs(100000); // 100ms message gap (> the 4.3ms header space)
  if (!rx.begin())
  {
