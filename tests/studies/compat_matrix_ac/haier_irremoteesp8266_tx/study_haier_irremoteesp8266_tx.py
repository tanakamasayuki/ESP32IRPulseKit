...
# AC decoder calibration (Haier): an INDEPENDENT transmitter (IRremoteESP8266's
# IRHaierAC) encodes a known standard 9-byte HAIER_AC state and transmits it; our
# RX (ESP32IRPulseKit, esp32irpk::ac::Haier) captures and decodes it. Proves our
# fromRaw() reads a third-party Haier double-header burst and recovers the same
# bytes + fields (including the inverted fan wire code).
#
...
#
# Both stacks use the same field codes (mode: auto0/cool1/dry2/heat3/fan4;
# fan: auto0/low1/med2/high3). HAIER_AC is command-based: power is the On/Off
# command, set last on the peer so power() = "not the Off command".
...
# 9 bytes = 18 hex chars. The trailing newline anchor is required so pexpect waits
# for the whole line instead of matching a partial hex prefix from the first chunk.
TX_OK_AC = re.compile(rb"TX_OK_AC vendor=HAIER bytes=(?P<bytes>[0-9A-Fa-f]{18})\r?\n")
AC_DECODE = re.compile(
    rb"AC_DECODE vendor=HAIER checksum=(?P<checksum>ok|bad) "
...
def test_haier_irremoteesp8266_tx(dut, peers, case, record_property):
