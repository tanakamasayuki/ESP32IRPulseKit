// Compat-AC field-map calibration (Midea) -- RX side (ESP32IRPulseKit).
//
// The peer TX (peer_tx/) is IRremoteESP8266's IRMideaAC, which encodes a KNOWN
// Midea state (48-bit / 6-byte) to its canonical bytes and transmits it (the data
// followed by an inverted copy). This primary captures that double burst with our
// RAW path and decodes it with esp32irpk::ac::Midea, printing the recovered 6
// bytes and our interpreted fields. The harness compares our bytes vs the peer's
...
{
  esp32irpk::ac::Midea::Frame f;
  if (!esp32irpk::ac::Midea::Frame::fromRaw(raw, f))
  {
...
void setup()
{
  ...
  rx.setMaxRxSymbols(1024);      // two 48-bit copies + headers/footers
  rx.setIdleThresholdUs(100000); // 100ms > the 5.6ms inter-copy gap
  if (!rx.begin())
  {
