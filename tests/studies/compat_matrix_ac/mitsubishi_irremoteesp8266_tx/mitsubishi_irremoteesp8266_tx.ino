// Compat-AC field-map calibration (Mitsubishi) -- RX side (ESP32IRPulseKit).
//
// The peer TX (peer_tx/) is IRremoteESP8266, which encodes a KNOWN Mitsubishi
// A/C state to its canonical 18 bytes and transmits it. This primary captures
// that burst with our RAW path and decodes it with esp32irpk::ac::Mitsubishi,
// printing both the recovered 18 bytes and our interpreted fields. The harness
// compares:
//   1. our bytes vs the peer's canonical bytes -> validates our RAW capture and
//      framing, independent of how we interpret the bytes;
//   2. our decoded fields vs the known sent state -> calibrates the field map
//      (byte/bit positions and mode/fan codes) in src/ac/Mitsubishi.h.
...
void setup()
{
  ...
  rx.setIdleThresholdUs(100000); // 100ms > 15.5ms inter-copy gap, < ~300ms ceiling
  if (!rx.begin())
  {
