...
# AC decoder calibration (Hitachi): an INDEPENDENT transmitter (IRremoteESP8266's
# IRHitachiAc) encodes a known standard 28-byte HITACHI_AC state and transmits it;
# our RX (ESP32IRPulseKit, esp32irpk::ac::Hitachi) captures and decodes it. Proves
# our fromRaw() reads a third-party Hitachi burst and recovers the same bytes +
# fields (including the per-byte bit-reversal and the sum checksum).
#
...
#
# Both stacks use the same field codes (mode: auto2/heat3/cool4/dry5/fan12;
# fan: auto1/low2/med3/high5). Fan mode carries a sentinel temperature (temp n/a).
...
OUR_MODE = {"AUTO": 2, "HEAT": 3, "COOL": 4, "DRY": 5, "FAN": 12}
OUR_FAN = {"AUTO": 1, "LOW": 2, "MED": 3, "HIGH": 5}

# 28 bytes = 56 hex chars. The trailing newline anchor is required so pexpect waits
# for the whole line instead of matching a partial hex prefix from the first chunk.
TX_OK_AC = re.compile(rb"TX_OK_AC vendor=HITACHI bytes=(?P<bytes>[0-9A-Fa-f]{56})\r?\n")
AC_DECODE = re.compile(
    rb"AC_DECODE vendor=HITACHI checksum=(?P<checksum>ok|bad) "
    rb"power=(?P<power>\d+) mode=(?P<mode>\d+) temp=(?P<temp>\d+) "
    rb"fan=(?P<fan>\d+) bytes=(?P<bytes>[0-9A-Fa-f]{56})\r?\n"
)
...
def test_hitachi_irremoteesp8266_tx(dut, peers, case, record_property):
    ...
    # Fan mode carries a sentinel temperature, so temp is a don't-care there.
    temp_part = field("temp", observed["temp"], case.temp) if case.mode != "FAN" else "temp=n/a"
    print(
