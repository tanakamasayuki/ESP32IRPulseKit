// Compat-AC field-map calibration (Hitachi) -- RX side (ESP32IRPulseKit).
//
// The peer TX (peer_tx/) is IRremoteESP8266's IRHitachiAc, which encodes a KNOWN
// Hitachi state (28-byte HITACHI_AC) to its canonical bytes and transmits it. This
// primary captures the single pulse-distance burst with our RAW path and decodes
// it with esp32irpk::ac::Hitachi, printing the recovered 28 bytes and our
// interpreted fields. The harness compares our bytes vs the peer's canonical bytes
// and our fields vs the known state.
...
{
  esp32irpk::ac::Hitachi::Frame f;
  if (!esp32irpk::ac::Hitachi::Frame::fromRaw(raw, f))
  {
  ...
  Serial.println();
  Serial.flush(); // the 28-byte line is long; drain to avoid ring-buffer truncation
}
} // namespace
...
void setup()
{
  ...
  rx.setMaxRxSymbols(1024);      // single 224-bit frame
  rx.setIdleThresholdUs(100000); // 100ms message gap
  if (!rx.begin())
  {
