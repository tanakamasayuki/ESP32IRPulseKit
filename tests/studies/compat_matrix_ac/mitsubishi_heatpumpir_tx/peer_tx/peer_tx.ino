// Compat-AC second reference (Mitsubishi) -- TX side (HeatpumpIR).
//
// Drives the IR LED with HeatpumpIR's MitsubishiFEHeatpumpIR over an LEDC carrier
...
// identity: HeatpumpIR uses a different 18-byte template).
//
// HeatpumpIR's Mitsubishi fan steps FAN_1..FAN_5 map to our LOW/MED/HIGH/MAX/QUIET,
// so this peer also exercises QUIET and HIGH, broadening fan coverage.
//
...
MitsubishiFEHeatpumpIR heatpump;
...
// HeatpumpIR Mitsubishi fan FAN_1..FAN_5 -> our LOW/MED/HIGH/MAX/QUIET.
bool mapFan(const String &name, uint8_t &fan)
{
  if (name == "AUTO")  { fan = FAN_AUTO; return true; }
  if (name == "LOW")   { fan = FAN_1;    return true; }
  if (name == "MED")   { fan = FAN_2;    return true; }
  if (name == "HIGH")  { fan = FAN_3;    return true; }
  if (name == "MAX")   { fan = FAN_4;    return true; }
  if (name == "QUIET") { fan = FAN_5;    return true; }
  return false;
}
...
void handleSendAc(const String &line)
{
  ...
  Serial.println("TX_OK_AC vendor=MITSUBISHI impl=HeatpumpIR");
}
