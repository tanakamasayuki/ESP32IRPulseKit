// Compat-AC second reference (Mitsubishi) -- RX side (ESP32IRPulseKit).
//
// The peer TX (peer_tx/) is HeatpumpIR's MitsubishiFEHeatpumpIR over an LEDC
// carrier -- a transmit path independent of both ESP32IRPulseKit and
// IRremoteESP8266. This primary captures the burst with our RAW path and decodes
// it with esp32irpk::ac::Mitsubishi. The harness checks our decoded fields
// against the known sent state (semantic agreement; byte identity is not expected
// since HeatpumpIR uses a different template than IRremoteESP8266).
...
{
  esp32irpk::ac::Mitsubishi::Frame f;
  if (!esp32irpk::ac::Mitsubishi::Frame::fromRaw(raw, f))
  {
...
void setup()
{
  ...
  rx.setMaxRxSymbols(1024);      // hold both repeated copies of the frame
  rx.setIdleThresholdUs(100000); // 100ms > inter-copy gap, < ~300ms ceiling
  if (!rx.begin())
  {
