// Compat-AC second reference (Gree) -- TX side (HeatpumpIR).
//
// Drives the IR LED with HeatpumpIR's GreeGenericHeatpumpIR over an LEDC carrier
...
// identity).
//
// HeatpumpIR's Gree has the same field layout and two-block framing as ours, but
// a few model quirks: MODE_AUTO forces 25C, MODE_DRY forces the low fan step.
//
...
GreeGenericHeatpumpIR heatpump;
...
// Gree exposes auto + three speeds; the generic FAN_1/2/3 map to low/med/high,
// which our decoder reads as MIN/MED/MAX.
bool mapFan(const String &name, uint8_t &fan)
{
  ...
  if (name == "MAX")  { fan = FAN_3;    return true; }
  return false;
}
...
void handleSendAc(const String &line)
{
  ...
  Serial.println("TX_OK_AC vendor=GREE impl=HeatpumpIR");
}
