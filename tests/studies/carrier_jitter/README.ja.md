...
マーク幅やキャリアを最適化すれば送信エッジ精度をさらに上げられる**可能性はある**が、現行
（RMT 10µsティック・プロトコル標準のタイミング・既定キャリア）でも**復調許容（NEC ±25%≈
±140µs）に対して十分な精度**があり、最適化は**複雑性を増すだけ**で実利に乏しい（10µsティック
では整数サイクル幅をそもそも表現できず、規格値から外せば相互運用も損なう）。よって
**プロトコル標準値を採用**する。

ただしキャリアのデューティだけは、環境（距離・角度・外乱光）に応じて利用側で調整できるよう
`IRSender::setCarrierDuty(float)` を追加した（既定 33% は不変。0<duty<1、次回送信から反映）。
周波数は従来どおり `setCarrierHz()`。
