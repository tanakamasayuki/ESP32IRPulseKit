...
IR link. Unlike `studies/tx_jitter/` (over-the-air, through an IR LED + TSOP
...
```sh
cd tests
uv run --env-file .env pytest -o python_files="study_*.py" studies/tx_jitter_loopback/
```
...
uv run python studies/tx_jitter_loopback/analyze.py

# Or point it at specific result dirs / dut.log files, and list more edges
uv run python studies/tx_jitter_loopback/analyze.py --worst 10 \
...
The same script also works on `studies/tx_jitter/` logs (same `RX_JITTER`
...
Methodology note: the over-the-air rig (`studies/tx_jitter/`) measured *larger*
