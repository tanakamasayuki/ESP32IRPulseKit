...
`studies/tx_jitter/`（IR LED＋TSOP受信モジュール経由の無線。キャリア/復調歪みが乗る）
と違い、本リグは:
...
同じスクリプトは `studies/tx_jitter/`（同じ `RX_JITTER` 形式）でも動くので、
...
手法メモ: 無線リグ（`studies/tx_jitter/`）では IR LED + TSOP のキャリア復調が支配的で、
