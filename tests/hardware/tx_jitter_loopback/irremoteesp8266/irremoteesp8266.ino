// Single-board carrier-off loopback jitter rig (TX: IRremoteESP8266).
//
// Transmits NEC on LOOPBACK_TX_GPIO with modulation disabled (use_modulation=
// false -> solid marks) and captures the wire-looped signal on LOOPBACK_RX_GPIO
// with the RMT peripheral at 1 MHz. Wire LOOPBACK_TX_GPIO -> LOOPBACK_RX_GPIO
// directly. See pulsekit/ for the rig rationale.
#include <IRsend.h>
...
// inverted=false, use_modulation=false -> no 38 kHz carrier (solid marks).
IRsend irsend(kIrTxGpio, false, false);
...
namespace
{
  ...
  void primeCapture()
  {
    ...
    irsend.sendNEC(0xCB3400FFULL, 32);
