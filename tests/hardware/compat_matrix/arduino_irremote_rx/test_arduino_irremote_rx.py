...
class Case:
    ...
    # Arduino-IRremote RX may decode it with a different value due to
...
def read_first_decode(rx, case: Case):
    try:
        ...
        pytest.fail(
            f"Arduino-IRremote RX saw no IR signal for {case.protocol} "
    ...
    if match.group("protocol") is None:
        ...
        pytest.fail(
            f"Arduino-IRremote RX received a frame (raw_len={raw_len}) but could "
...
def test_pulsekit_tx_to_arduino_irremote_rx(dut, peers, case, record_property):
    ...
    record_property("rx_impl", "Arduino-IRremote")
    ...
    print(
        "COMPAT_MATRIX_OBSERVED rx=Arduino-IRremote "
