...
class Case:
    # protocol: keyword the Arduino-IRremote TX peer understands.
    # bits: payload handed to the external library (sent MSB-first). The
    # ESP32IRPulseKit RX may decode it with a different value due to
...
def read_first_decode(rx, case: Case):
    try:
        ...
        pytest.fail(
            f"RX did not decode anything for Arduino-IRremote {case.protocol} "
...
def test_arduino_irremote_tx_to_pulsekit_rx(dut, peers, case, record_property):
    ...
    record_property("tx_impl", "Arduino-IRremote")
    ...
    print(
        "COMPAT_MATRIX_OBSERVED tx=Arduino-IRremote "
