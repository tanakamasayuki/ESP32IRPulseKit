...
ACのサンプルは、位相整合の既定ではなくハードウェアキャリア
（`setPhaseAlignedCarrier(false)`）を選ぶ——必然ではなくコストの観点から。
キャリア1周期あたり約1シンボルなので、複数フレームのACバーストは数千シンボル
（約17KB）に膨らみ、一時確保してチャンネルバッファ経由でストリームする。位相整合
エンコーダでも正しく送信でき——15bitフィールドを超える時間は複数シンボルに分割
されるので、セクション間ギャップ（そもそも1us分解能の単一シンボル上限~33ms内）も
制約にならない——実測の到達率もハードウェアキャリアと同等だった
（`tests/studies/compat_matrix_ac/irremoteesp8266_rx/study_carrier_ab.py`：
位相整合・ハードウェアとも 150/150）。位相整合の方がマーク精度は高い（±1周期の
揺れが無い）が、1送信あたりの大きな確保と重い割り込み負荷下でのリフィルアンダーラン
リスクのため、サンプルでは長尺フレームにハードウェアキャリアを推奨する。真の
ボトルネックはマーク展開（シンボル数）であり、ギャップの表現ではない。
