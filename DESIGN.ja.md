...
- **常用の制御フィールドにはアクセサを付ける**: power/mode/温度/fan は実装済。温度は
  温度は `temperatureC()`/`setTemperatureC(float)` の対称な float ペアで0.5℃刻み
  （+0.5 は byte22 bit7、`halfDegree()` は簡便な読み）、風向 `louver()` も実装済。
  しずか/パワフルは保留中（最後の項目参照）。
