...
長尺のエアコンフレームは、必然ではなくコストの観点で既定をハードウェアキャリアに
する。キャリア1周期あたり約1シンボルなので、複数フレームのACバーストは数千シンボル
...
位相整合・ハードウェアとも 150/150）。既定にする理由は、1送信あたりの大きな確保と
重い割り込み負荷下でのリフィルアンダーランのリスクで、ハードウェアキャリアはこれを
避けられるため。真のボトルネックはマーク展開（シンボル数）であり、ギャップの表現
ではない。
