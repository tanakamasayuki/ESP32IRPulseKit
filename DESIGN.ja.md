...
長尺のエアコンフレームは、ハードウェアキャリアを*使わざるを得ない*唯一のケース：
キャリア1周期あたり約1シンボルなので、複数フレームのACバーストはチャンネルが
ストリームできる量を遥かに超える。よってこの長さのフレームに位相整合エンコーダは
使えない。代償は同じ±1周期の揺れで、同一受信側に対する IRremoteESP8266 純正TXより
1フレームあたり到達率がわずかに低い形で現れる
（`tests/studies/compat_matrix_ac/irremoteesp8266_rx`：23/25 対 25/25、破損ではなく
ドロップ）。この差を埋める——長尺に収まる位相整合経路、例えばキャリアを事前展開せず
送信中にライブエンコードする方式——は今後の課題。
