...
namespace esp32irpk::specs
{
  ...
  // The bit-mark and zero-space are both 525 us; the one-space is 1575 us.
  //
  // Repeats: a real JVC remote sends the 8400/4200 header only once and then
  // resends the 16-bit data block *without* the header for held-button repeats.
  // PulseKit models a header-led 16-bit frame (NORMAL) each time; the headerless
  // repeat form is not a distinct frame type and still decodes on standard
  // receivers.
